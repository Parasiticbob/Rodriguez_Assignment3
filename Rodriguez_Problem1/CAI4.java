...
public class CAI4 {
	...
	public static void main(String[] args) {
		...
		while(true) {
			counter = 0;
			System.out.printf("%n%nNew Quiz!! Choice a Difficulty(1, 2, 3 or 4): ");
			readDifficulty();
			quiz();
		}
	...
	public static double randomNumTwo() {//Randomly produces an int between 0 - 9
		int max = 99, min = 0;
		double x = (int) (Math.random() * ((max - min) + 1)) + min;
		return x;
	}
	public static double randomNumThree() {//Randomly produces an int between 0 - 9
		int max = 999, min = 0;
		double x = (int) (Math.random() * ((max - min) + 1)) + min;
		return x;
	}
	public static double randomNumFour() {//Randomly produces an int between 0 - 9
		int max = 9999, min = 0;
		double x = (int) (Math.random() * ((max - min) + 1)) + min;
		return x;
	}
